...
$scrwd = 150;      // Scroller width in pixels
...
$a = 1;
$content = '';
$result = $db->sql_query('SELECT `lid`, `title` FROM `' . $prefix . '_nsngd_downloads` ORDER BY `lid` DESC LIMIT 0, ' . $blkh);
$numrows = $db->sql_numrows($result);
if ($numrows == 0) {
	$content .= _BLOCKPROBLEM2;
	} else {
	$content = '<div class="' . $ListClass . ' block-nsngd_new">';
	if ($scron == 1) {
