...
echo '<p><a class="rn_csrf" href="' . $admin_file . '.php?op=FilesizeValidate&amp;cid=0">' . _DL_CHECKALLDOWNLOADS . '</a></p>';
...
if ($db->sql_numrows($result) > 0) {
	...
	while ($cidinfo = $db->sql_fetchrow($result)) {
		...
		echo '<li><a class="rn_csrf" href="' . $admin_file . '.php?op=FilesizeValidate&amp;cid=' . $cidinfo['cid'] . '">'
			. $cidtitle . '</a></li>';
	}
	echo '</ul>';
}
