...
function of_group($gid) {
	...
	// If you wish for an admin to always return a "true" response, uncomment the below code
	/*if (is_admin($admin)) {
	return 1;
	} else*/
	if (is_user($user)) {
		...
		$guid = $cookie[0];
		$currdate = time();
		$sql = 'SELECT * FROM `' . $prefix . '_nsngr_users` WHERE `gid` = ' . (int)$gid . ' AND `uid` = ' . (int)$guid
			. ' AND (`edate` > \'' . $currdate . '\' OR `edate` = \'0\')';
		$ingroup = $db->sql_numrows($db->sql_query($sql));
		if ($ingroup > 0) {
			return 1;
		}
	}
...
function restricted($perm) {
	...
	if ($perm == 1) {
		...
		$newView = $perm-2;
		$sql = 'SELECT `gname` FROM `' . $prefix . '_nsngr_groups` WHERE `gid` = ' . $newView;
		list($who_view) = $db->sql_fetchrow($db->sql_query($sql));
		$who_view = $who_view . ' ' . _DL_ONLY;
	}
	$myimage = myimage('restricted.png');
	echo '<div align="center"><p><img src="' . $myimage . '" alt="" /></p>';
	echo '<p>' . _DL_DENIED . '!</p>';
...
function newdownloadgraphic($datetime, $time) {
	...
	while ($count <= 14) {
		...
		if ($daysold == $datetime) {
			...
			if ($count <= 1) {
				echo '<img align="middle" src="' . $myimage . '" alt="' . _DL_NEWTODAY . '" title="' . _DL_NEWTODAY . '" />';
			}
			$myimage = myimage('new_03.png');
			if ($count <= 3 && $count > 1) {
				echo '<img align="middle" src="' . $myimage . '" alt="' . _DL_NEWLAST3DAYS . '" title="' . _DL_NEWLAST3DAYS . '" />';
			}
			$myimage = myimage('new_07.png');
			if ($count <= 7 && $count > 3) {
				echo '<img align="middle" src="' . $myimage . '" alt="' . _DL_NEWTHISWEEK . '" title="' . _DL_NEWTHISWEEK . '" />';
			}
			$myimage = myimage('new_14.png');
			if ($count <= 14 && $count > 7) {
				echo '<img align="middle" src="' . $myimage . '" alt="' . _DL_NEWLAST2WEEKS . '" title="' . _DL_NEWLAST2WEEKS . '" />';
			}
		}
		$count++;
...
function popgraphic($hits) {
	...
	if ($hits >= $dl_config['popular']) {
		echo '&nbsp;<img align="middle" src="' . $myimage . '" alt="' . _DL_POPULAR . '" title="' . _DL_POPULAR . '" />';
	}
}
...
function menu($maindownload) {
	global $module_name;
	OpenTable();
	$myimage = myimage('down-logo.png');
	echo '<div align="center" class="content"><p><a href="modules.php?name=' . $module_name . '"><img src="' . $myimage . '" border="0" alt="" title="" /></a></p>';
	SearchForm();
	echo '<p>[ ';
	if ($maindownload > 0) {
	...
	echo '<tr>';
	$myimage = myimage('new_01.png');
	echo '<td align="center" width="33%"><img align="middle" src="' . $myimage . '" alt="" title="" />=' . _DL_NEWTODAY . '</td>';
	$myimage = myimage('new_03.png');
	echo '<td align="center" width="34%"><img align="middle" src="' . $myimage . '" alt="" title="" />=' . _DL_NEWLAST3DAYS . '</td>';
	$myimage = myimage('new_07.png');
	echo '<td align="center" width="33%"><img align="middle" src="' . $myimage . '" alt="" title="" />=' . _DL_NEWTHISWEEK . '</td>';
	echo '</tr>';
	echo '<tr>';
	$myimage = myimage('new_14.png');
	echo '<td align="center" width="33%"><img align="middle" src="' . $myimage . '" alt="" title="" />=' . _DL_NEWLAST2WEEKS . '</td>';
	echo '<td align="center" width="34%">&nbsp;</td>';
	$myimage = myimage('popular.png');
	echo '<td align="center" width="33%"><img align="middle" src="' . $myimage . '" alt="" title="" />=' . _DL_POPULAR . '</td>';
	echo '</tr>';
	echo '</table>';
	CloseTable();
}
	...
	if (($lidinfo['sid'] == 0) || ($lidinfo['sid'] == 1 AND is_user($user)) || ($lidinfo['sid'] == 2 AND is_admin($admin)) || ($lidinfo['sid'] > 2 AND of_group($priv)) || $dl_config['show_download'] == '1') {
		if (is_admin($admin)) {
			...
			echo '<a class="rn_csrf" href="' . $admin_file . '.php?op=DownloadModify&amp;lid=' . $lid . '"><img align="middle" src="'
				. $myimage . '" border="0" alt="' . _DL_EDIT . '" /></a>&nbsp;';
		} else {
			$myimage = myimage('show.png');
			echo '<img align="middle" src="' . $myimage . '" border="0" alt="" />&nbsp;';
		}
		echo '<a href="modules.php?name=' . $module_name . '&amp;op=getit&amp;lid=' . $lid . '"><strong>'
			. htmlspecialchars($lidinfo['title'], ENT_QUOTES, _CHARSET) . '</strong></a>';
		...
		if (preg_match('#^(http(s?))://#i', $lidinfo['homepage'])) {
			echo '<a href="' . $lidinfo['homepage'] . '" target="_blank">' . htmlspecialchars($lidinfo['homepage'], ENT_QUOTES, _CHARSET) . '</a>';
		} else {
			echo _DL_NOTLIST;
		}
	} else {
	...
	if (($lidinfo['sid'] == 0) || ($lidinfo['sid'] == 1 AND is_user($user)) || ($lidinfo['sid'] == 2 AND is_admin($admin)) || ($lidinfo['sid'] > 2 AND of_group($priv)) || $dl_config['show_download'] == '1') {
		if (is_admin($admin)) {
			...
			echo '<a class="rn_csrf" href="' . $admin_file . '.php?op=DownloadModify&amp;lid=' . $lid . '"><img align="middle" src="' . $myimage
				. '" border="0" alt="' . _DL_EDIT . '" title="' . _DL_EDIT . '" /></a>&nbsp;';
		} else {
			$myimage = myimage('show.png');
